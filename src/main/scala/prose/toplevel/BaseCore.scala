...
/** Base Systolic array core without any additional functionalities
  * @param kMax
  *   maximum length row/column stream to stream in
  * @param Nmin
  *   min dimension of systolic array on the accelerator. This determines
  *   alignment
  * @param N
  *   size of systolic array in this core
  * @param SRAMLatency
  *   SRAM latency in cycles
  * @param fpuLatency
  *   FPU FMA latency in cycles
  * @param maxBatch
  *   maximum batch size. Minimum batch is determined by Nmax / N
  * @param specialFunc
  *   special operation with latency
  * @param p
  *   constructor argument for Beethoven
  */
abstract class BaseCore(
  ...
  class MatrixOpCmd extends AccelCommand("matrixOp") {
    ...
    val outputRowOffset = UInt(
      addrBits.W
    ) // byte offset for a stripe in the output matrix
    // ---------- CONTROL ----------- //
    ...
    // ---------- SOFTMAX ----------- //
    val softmax_out =
      if (specialFunc.exists(_._1 == SpecialFunction.EXP)) Some(Address())
      else None
    // ---------- ACTIVATIONS ----------- //
  ...
  /** For test-chip purposes, we want to be able to test the power-consumption
    * of the chip under load which we can't actually due with the HyperRAM
    * bandwidth (300MBs). To get around this, we generate random normal numbers.
    * Box-Mueller too expensive so we do 1cy LFSR + 2cy LUT but we can do this
    * at a throughput of 1/cy
    */
  def getReaderWrapper(
      name: String
  ): (List[DecoupledIO[ChannelTransactionBundle]], List[DataChannelIO]) = if (
    useFakeLUTs
  ) {
    val (reqs, dats) = getReaderModules(name)
    reqs.zipWithIndex.foreach(a =>
      a._1.suggestName(f"requestChannel_${name}_${a._2}")
    )
    dats.zipWithIndex.foreach(a =>
      a._1.suggestName(f"dataChannel_${name}_${a._2}")
    )
    ...
    val req_wrapper = reqs.zip(dats).zipWithIndex.map { case ((r, d), idx) =>
      ...
      val randomFloatGen = (0 until dwidth / 16) map { _ =>
        val perSlotUInt = VecInit((0 until 16) map { _ =>
          val lfsr =
            Module(new LinearFeedbackShiftRegister(13, List(0, 1, 2, 7)))
  ...
  val (activations_out_req_left, activations_out_left) = getWriterWrapper(
    "activation_out"
  )
  ...
  val s_idle :: s_start_row :: s_emit_weight_request :: s_launch_mac :: s_mac :: s_flush :: s_finish :: s_aux :: Nil =
    Enum(8)
  ...
  active_cores_MO := Mux(
    CTRL_cmdCopy.nRowTilesToDo_MO >= n_arrays_mo.U,
    n_arrays_mo.U,
    CTRL_cmdCopy.nRowTilesToDo_MO
  )
  ...
  val norm_is_ready = {
    ...
    when(norm_state === norm_idle) {
      when(state === s_start_row) {
        ...
        val norm_req_len = (N * 2 * (n_arrays)).U * Mux(
          w_norm_batched,
          CTRL_cmdCopy.batchSize,
          1.U
        )
  ...
  val bias_streams = {
    ...
    when(state === s_start_row) {
      if (!supportWideBias) {
        assert(
          wire_b_mode =/= biasMATRIX.U && wire_b_mode =/= biasBATCHEDMATRIX.U,
  ...
  println(
    f"${outer.systemParams.name} has memory size ${kMax * maxBatch} x ${N * 16 * n_arrays}"
  )
  ...
  def parse(a: Seq[UInt]): Vec[UInt] = VecInit(
    a.reverse.flatMap(q => splitToChunks(q, 16))
  )

  val activationFIFO = Module(
    new Queue(Vec(N * n_arrays, UInt(16.W)), SRAMLatency + 1, hasFlush = true)
  )
  ...
  val arrays = Seq.tabulate(n_arrays) { array_idx =>
    val array =
      Module(new SystolicArray(N, kMax, maxBatch, useUpShift, fpuLatency))
    ...
    val weightsSplit = VecInit(
      weights.reverse.flatMap(q => splitToChunks(q.data.bits, 16)).reverse
    )
    (0 until N) foreach { i =>
  ...
  when(activeWriteToActivationBuffer) {
    actChosen := actValue
    activations_valid := activations.zipWithIndex
      .map { case (act, act_idx) =>
    ...
    activation_buffer.addr(0) := actReadCnt
    val read_buffer_valid =
      state === s_mac && activationSpace > 0.U && !activeWriteToActivationBuffer
    ...
    activationFIFO.io.enq.valid := ShiftReg(
      read_buffer_valid,
      SRAMLatency,
      clock
    )
    activationFIFO.io.enq.bits := splitToChunks(
      activation_buffer.data_out(0),
      16
    ).reverse
  ...
  val simd_engine = Module(
    new ProseSIMD(N * n_arrays, simdLatency, true, 16, specialFunc)
  )
  val bias_data_valid = Wire(Bool())
  val can_consume_array_output =
    simd_engine.io.readyIn && simd_engine.io.validIn
  ...
  when(biasMode === biasCOLS.U) {
    ...
    bias_data_valid := bias_streams.zipWithIndex
      .map { case (bstream, stream_idx) =>
  ...
  val biasColChoice =
    splitIntoChunks(bias_streams(0).data.bits, 16)(biasReadCtr)
  ...
  when(SIMD_inFire) {
    ...
    when(nxt === activationBatch || biasMode === biasBATCHEDMATRIX.U) { // if the batch shares the bias then we need to count
      ...
      when(
        biasReadCtr === (Nmin - 1).U || biasMode === biasBATCHEDMATRIX.U || biasMode === biasMATRIX.U
      ) {
        biasReadCtr := 0.U
  ...
  simd_engine.io.operandMult.get
    .zip(norms.map(_.apply(norm_counter_batch))) foreach { case (sink, src) =>
    sink := src
    when(!useNorms) {
      sink := 0x3f80.U
    }
  ...
  val simd_extra_slack_queue = Module(
    new Queue[Vec[UInt]](simd_engine.io.out.cloneType, 2)
  )
  ...
  simd_extra_slack_queue.io.deq.ready := activations_out_left.zipWithIndex
    .map { case (out_stream, stream_idx) =>
      val array_idx = stream_idx / (N / Nmin)
      out_stream.data.ready || array_idx.U > active_cores_MO
    }
    .fold(true.B)(_ && _)

  activations_out_left
    .zip(simd_extra_slack_queue.io.deq.bits.grouped(Nmin))
    .zipWithIndex foreach { case ((o_stream, dat), stream_idx) =>
    val array_idx = stream_idx / (N / Nmin)
  ...
  def increment_on_row_incr(): Unit = {
    ...
    require(
      isPow2(n_arrays),
      "need n_arrays to be power of 2 to simplify address computation. Can change implementation to not need this"
    )
    when(CTRL_cmdCopy.outputTranspose.getOrElse(true.B)) {
      CTRL_cmdCopy.addrOut := CTRL_cmdCopy.addrOut +
        Misc.multByIntPow2(
          CTRL_cmdCopy.batchSize * Misc.multByIntPow2(
            CTRL_cmdCopy.nColTilesToDo_MO +& 1.U,
            N
          ),
          N * 2 * n_arrays
        )
    }.otherwise {
      CTRL_cmdCopy.addrOut := CTRL_cmdCopy.addrOut + Misc.multByIntPow2(
        CTRL_cmdCopy.batchSize,
        2 * N * Nmin * n_arrays
      )
    }
    ...
    when(
      CTRL_cmdCopy.bias_mode === biasMATRIX.U || CTRL_cmdCopy.bias_mode === biasBATCHEDMATRIX.U
    ) {
    ...
    CTRL_cmdCopy.activationAddr := CTRL_cmdCopy.activationAddr +
      Misc.multByIntPow2(
        CTRL_cmdCopy.matrixLen * CTRL_cmdCopy.batchSize,
        N * n_arrays * 2
      )
  ...
  when(state === s_idle) {
    ...
    val readLenBatched = Misc.multByIntPow2(
      CTRL_cmdCopy.matrixLen * CTRL_cmdCopy.batchSize,
      Nmin * 2
    )
    ...
    activations_out_req_left.zipWithIndex.foreach { case (w_o, idx) =>
      val array_idx = idx / (N / Nmin)
      w_o.valid := CTRL_cmdCopy.outputTranspose.getOrElse(
        true.B
      ) && (active_cores_MO >= array_idx.U)
      val wLen =
        (Nmin * N * 2).U * CTRL_cmdCopy.batchSize * (1.U +& CTRL_cmdCopy.nColTilesToDo_MO)
    ...
    val readLenBatched =
      Misc.multByIntPow2(sharedDim * activationBatch, Nmin * 2)
    val readLen =
      Mux(CTRL_cmdCopy.weightsAreBatched, readLenBatched, readLenNonBatched)
    ...
    val outs_ready = if (useUpShift) {
      ...
      activations_out_req_left
        .map(_.ready)
        .fold(true.B)(_ && _) || CTRL_cmdCopy.outputTranspose.get
    } else true.B
    ...
    when(can_emit) {
      state := s_launch_mac
      CTRL_weightsBaseAddressAccumulator := CTRL_weightsBaseAddressAccumulator + Misc
        .multByIntPow2(readLen, N / Nmin)
      ...
      CTRL_outputsBaseAddressAccumulator := CTRL_outputsBaseAddressAccumulator +
        Misc.multByIntPow2(
          Mux(
            CTRL_cmdCopy.outputTranspose.getOrElse(true.B),
            wLen,
            CTRL_cmdCopy.outputRowOffset
          ),
          N / Nmin
        )
    }
    ...
    when(CTRL_nColTiles_MO_accumulator =/= 0.U) {
      ...
      val req_complete =
        activations_out_req_left.map(_.ready).fold(true.B)(_ && _)

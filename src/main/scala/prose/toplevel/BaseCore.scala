...
abstract class BaseCore(kMax: Int,
  ...
  def increment_on_row_incr(): Unit = {
    ...
    // don't modify weight accumulator because that's managed in the s_start_new_row state
    // don't modify norm addr because it was already incremented
    ...
    //     stripe_size =  K * PROSE_Nmin * 2 * chosen_batch_size;
    //     act_acc = act_acc +
    //        activation_stripe_size_bytes * (PROSE_ECore_N / PROSE_Nmin) * to_do;
    // total = K * E_N / Nmin * Nmin * 2 * batch * n_arrays
    //       = K * N * 2 * batch * n_arrays
    // batch and K are software inputs
    // N, 2, n_arrays are constants

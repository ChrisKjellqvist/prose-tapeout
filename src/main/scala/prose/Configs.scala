...
case class SystolicType(nCores: Int,
  ...
  def toConfig(Nmin: Int,
               SRAMLatency: Int,
               fpuLatency: Int,
               maxBatchSize: Int,
               maxNColsTiles: Int): AcceleratorSystemConfig = {
    CppGeneration.addPreprocessorDefinition(s"NCORES_${namePrefix}", nCores)
    AcceleratorSystemConfig(nCores = 1,
      name = f"${namePrefix}Core",
      moduleConstructor = ModuleBuilder({ p =>
        specialFunc match {
          case Some((SpecialFunction.EXP, latency)) => new EType(latency)(maxMatrixLen, Nmin, N, SRAMLatency, fpuLatency, fpuLatency + 1, maxBatchSize, maxNColsTiles, supportWideBias, nCores)(p)
          case Some((SpecialFunction.GELU, latency)) => new GType(latency)(maxMatrixLen, Nmin, N, SRAMLatency, fpuLatency, fpuLatency + 1, maxBatchSize, maxNColsTiles, supportWideBias, nCores)(p)
          case None => new MType()(maxMatrixLen, Nmin, N, SRAMLatency, fpuLatency, fpuLatency + 1, maxBatchSize, maxNColsTiles, supportWideBias, nCores)(p)
        }
      }),
...
class WithProse(cores: List[SystolicType],
  ...
  cores.map(a => a.toConfig(smallestN, SRAMLatency, fpuLatency, maxBatchSize, (maxNDim.toFloat / a.N).ceil.toInt))
})

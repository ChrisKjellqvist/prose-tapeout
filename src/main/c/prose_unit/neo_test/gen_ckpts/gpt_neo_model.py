...
class GPTNeoModel(nn.Module):
    ...
    def forward(
        ...
        for i, (block, _) in enumerate(zip(self.h, self.h)):
            outputs = block(
                hidden_states,
                layer_past=None,
                attention_mask=None,
...
state_dict['output_block'] = sample_output  # The first element is hidden_states

print(state_dict.keys())

for name, param in state_dict.items():
    # Save to model_ckpts/
    torch.save(param.cpu().detach(), f"model_ckpts/model_{name}.pt")

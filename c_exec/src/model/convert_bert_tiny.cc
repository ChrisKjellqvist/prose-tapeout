...
std::vector<std::pair<std::string, std::vector<int>>> get_index() {
  // Load the index file
  std::ifstream file("tinybert/layer_dict.txt");
...
int main() {
  ...
  for (const auto &pair: index) {
    ...
    auto matrix = load_matrix("tinybert/" + pair.first + ".npy", rows, cols);
  ...
  write_to_file("tinybert/prose_input.bin", matrices, {std::make_pair("tinybert/prose_index.txt", names_only)});
